...
public class S3Uploader implements Uploader {
    ...
    public void upload(List<String> files) {
        ...
        for (String filePath : files) {
            ...
            PutObjectResponse response = client.putObject(request, new File(filePath).toPath());

            if (response.sdkHttpResponse().isSuccessful()) {
                System.out.println("Successful uploaded file " + filePath + ". ETag: " + response.eTag());
                count ++;
            } else {
                System.out.println("Error uploading file " + filePath + ". Exception: " + response.sdkHttpResponse().statusCode());
            }
        }
